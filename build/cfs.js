...
(function() {
  var cfs, constants, fs, isWindows, log, path, touch;
  ...
  cfs.ensureFileExists = function(file) {
    if (fs.existsSync(file)) {
      return;
    }
    touch.sync(file);
  ...
  cfs.readdirSyncRecursive = function(baseDir) {
    ...
    fileList = readdirSyncRecursive(baseDir).map(function(val) {
      return path.relative(baseDir, val);
    });
    return fileList;
  };

  cfs.rmdirSyncRecursive = function(dir, failSilent) {
    var currFile, err, file, files, i, len;
    try {
  ...
  cfs.mkdirSyncRecursive = function(dir, mode) {
    var err, parentDir, slashIdx;
  ...
  cfs.chmodSyncRecursive = function(sourceDir, filemode) {
    var currFile, file, files, i, len;
